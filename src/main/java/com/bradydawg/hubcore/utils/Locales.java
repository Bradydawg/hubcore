...
public enum Locales {
    ...
    UPDATE_FOUND("Messages.UpdateFound", "&a[Chat]&7 A new update has been found on SpigotMC. Current version: %oldversion New version: %newversion"),
    UPDATE_DOWNLOADED("Messages.UpdateDownloaded", "&a[Chat]&7 A new update was downloaded and installed from SpigotMC. It will apply on the next server restart.");

    private final String value;
    private final String path;
    private static YamlConfiguration cfg;
    private static final File localeFolder = new File(HubCore.getInstance().getDataFolder(), "locales");
    ...
    public static void load() {
        ...
        if (!f.exists()) {
            try {
                HubCore.getInstance().saveResource("locales" + File.separator + Config.LOCALE.getString() + ".yml", true);
                File locale = new File(HubCore.getInstance().getDataFolder(), Config.LOCALE.getString() + ".yml");
                if (locale.exists()) {
                    locale.delete();
                }
                reload(false);
