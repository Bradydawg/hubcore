...
public class Utils {
    ...
    public static String translateColorCodes(String string, Player p) {
        return p.hasPermission("hubcore.chat.color") ? replaceColors(string) : string;
    }
    ...
    public static boolean checkForAds(String msg, Player p) {
        if (p.hasPermission("hubcore.bypassads")) {
            return false;
        }
        ...
        if (found) {
            for (Player op : HubCore.getInstance().getServer().getOnlinePlayers()) {
                if (!op.hasPermission("hubcore.notifyad")) {
                    continue;
                }
